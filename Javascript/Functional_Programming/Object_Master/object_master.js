...
const namesIdOver99 = pokemon.filter(p => p.id > 99).map(p => p.name);
console.log(namesIdOver99);

const onlyPoison = pokemon.filter(p => p.types.includes("poison") && p.types.length === 1).map(p => p.name);
